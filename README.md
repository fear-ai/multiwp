...
- `scripts/Scripts.md`: Script catalog, roles, settings, and conventions.
- `scripts/Options.md`: Authoritative option and environment-variable definitions; entries match each script's `usage()`.
- `scripts/Generated.md`: Option and helper cross-references. Generated by `scripts/gen-crossref.sh`; do not edit by hand.
- `RulesCf.md`: Design and workflows for the `rules-cf.sh` ruleset get/put/copy tool.
...
- Listed here: documents that are committed, describe how to run or extend the
  system, and are intended for anyone working on the repo.
- Not listed: generated files (they name their generator instead), per-directory
  change logs (they sit beside what they describe), operator inventories holding
  host-specific detail, and transient working notes. A document that is
  gitignored must not be referenced from a committed file.
...
- `scripts/Scripts.md`: script catalog, roles, settings, conventions.
- `scripts/Options.md`: option and environment-variable definitions.
- `RulesCf.md`: Cloudflare ruleset tool design and file formats.
