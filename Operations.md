...
- **Rate limiting** (`http_ratelimit`, ref `rl_alternat_apex`): **4 requests per 10s** per `ip.src` + `cf.colo.id`, action `block`. Rate limiting evaluates before the managed ruleset, so a flood is answered with a cheap 429 instead of a WAF-evaluated 403. The threshold is deliberately tight: the hub serves a single self-contained page (inline CSS, no subresources, no favicon), so a genuine page view costs one request and an alias redirect adds one more. Measured effect of tightening from 50: a 120-request concurrent burst went from 51 × 429 to 115 × 429 at a limit of 2/10s; 4/10s sits between, keeping the bulk of that mitigation while leaving headroom for a viewer who reloads.
  - **Known tradeoff:** requests beyond the fourth within 10s from one address are blocked, so multiple users behind a shared NAT egress (office, mobile carrier, household) can still collide, though 4/10s tolerates a reload or a second viewer where 2/10s did not. This is accepted for a redirect hub, where a blocked viewer simply retries; do not copy this threshold to a zone serving a real site.
