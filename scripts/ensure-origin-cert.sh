...
#
# Usage:
#   ./ensure-origin-cert.sh [--ssl-dir DIR] example.com [www.example.com ...]
#
# Behavior:
...
usage() {
    cat <<'EOF'
Usage: ensure-origin-cert.sh [--ssl-dir DIR] domain1 [domain2...]
...
sudo chmod 755 "$SSL_CERT_DIR" || true
sudo chmod 710 "$SSL_KEY_DIR" || true

ensure_perms() {
    sudo chown root:ssl-cert "$1" || true
    sudo chmod 640 "$1"
}
